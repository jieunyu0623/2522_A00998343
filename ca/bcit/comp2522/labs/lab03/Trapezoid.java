...
/**
 * Trapezoid class for finding area of a Trapezoid.
...
public class Trapezoid extends Quadrilateral {
   ...
   */
  public Trapezoid(final int x1, final int y1, final int x2,
   ...
   * @return ((top + base) / 2) * height double.
   */
  @Override
  public double area() {
    double top = Math.sqrt(Math.pow((getP1().getX() - getP2().getX()), 2)
            + Math.pow((getP1().getY() - getP2().getY()), 2));
    double base = Math.sqrt(Math.pow((getP3().getX() - getP4().getX()), 2)
